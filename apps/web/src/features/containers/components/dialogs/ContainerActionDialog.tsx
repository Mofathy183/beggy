...
import {
	...
	UnpackItemForm,
	MoveItemForm,
} from '@/features/containers/components/forms';
...
const DIALOG_DESCRIPTIONS: Record<ContainerDialogMode['mode'], string> = {
	pack: 'Choose what to add and how many.',
	unpack: 'How many are you taking out?',
	move: 'Pick where this item is heading.',
};
...
const ContainerActionDialog = ({
	...
	return (
		<ManagedFormDialog
			...
			title={title}
			description={DIALOG_DESCRIPTIONS[modeProps.mode]}
			size="sm"
			scrollable={false}
			form={(onCancel) => {
				const handleSuccess = () => {
