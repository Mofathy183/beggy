...
import {
	...
	ItemsPanel,
} from '@features/containers/components/visualization';
import { PackedItemList } from '@features/containers/components/list';
import { ContainerActionDialog } from '@features/containers/components/dialogs';

import { useContainerState } from '@features/containers/hooks';
...
const ContainerDetailPage = ({
	...
	const packedItems = containerState?.items ?? [];
	...
	return (
		...
		>
			{/*
			 * min-h-full + flex-col so the sticky footer always sits at
			 * the bottom even when there's little content.
			 * The AppShell content area must be overflow-y-auto for
			 * sticky to work — if it's overflow-hidden, use fixed instead.
			 */}
			<div className="flex flex-col min-h-full">
				<div className="flex flex-col gap-6 p-4 pb-24">
					...
					{/* ── Status panel ─────────────────────────────────── */}
					<ContainerStatusPanel
						status={containerState?.status ?? null}
						maxWeight={maxWeight}
				...
				{/* ── Sticky footer — sidebar-agnostic ─────────────── */}
				<div
					className={cn(
						'sticky bottom-0',
				...
				>
					...
					<Sheet>
						...
						>
							<SheetHeader className="shrink-0 px-5 pb-2 pt-1">
